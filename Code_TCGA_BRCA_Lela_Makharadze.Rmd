...
Thus, no files are attached to the project (working directory can be freely chosen).
...
kplotAll <- ggplot(data = PCAall[[4]], aes(x = PC1, y = PC2, color=resultsAll$cluster))+ xlab(paste("PC1 (", PCAall[[3]][1], "%)"))  + ylab(paste("PC2 (", PCAall[[3]][2], "%)"))+geom_point()+theme_bw()+theme(legend.position = "none") +scale_color_manual(values = c("deepskyblue1", "purple","red"))
                 
#k-means for all 500 samples overlayed with normal samples
kplotAllHighlight <- kplot+ geom_point(data=PCAall[[4]][samplesNT,], aes(x = PC1, y = PC2), color="blue")+theme(legend.position = "none")
...
kplotTP <-ggplot(data = PCATP[[4]], aes(x = PC1, y = PC2, color=resultsTP$cluster ))+ xlab(paste("PC1 (", PCATP[[3]][1], "%)"))  + ylab(paste("PC2 (", PCATP[[3]][2], "%)"))+geom_point()+theme_bw()+theme(legend.position = "none") +scale_color_manual(values = c("red","purple"))
...
Using *MultiAssayExperiment* package, methylation data with the matched samples from the gene expression data was obtained and 
aligned with the transcriptome data.
...
```{r, unspurevised clustering of methylome, eval=FALSE, echo=T}
...
Methylation levels were tested for differentially expressed genes to find a correlation between gene expression status and methylation
of CpGs associated with them. To this end, several additional operations were required to find matching probes for the genes of interest.

```{r, matching probes to genes, eval=F, echo=T}
...
```{r, code for boxplots, eval=F, echo=T}
